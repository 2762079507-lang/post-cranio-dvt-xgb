...
## 与旧版 Decision Tree 应用的关键区别

| 项目 | 旧版 (dvt-predictor-dt) | 本版 (XGBoost) |
|------|------------------------|----------------|
| 算法 | Decision Tree (max_depth=5) | XGBoost (gradient-boosted trees) |
| 插管变量 | `ETT_48H`（术后 48h 插管） | `ETT_24H`（术后 24h 插管/机械通气） |
| 阈值 (cutoff) | 0.2995 | **0.0728**（内部验证集锁 90% 灵敏度） |
| 阈值锁定集 | 训练集 | **内部验证集**（避免训练集过拟合导致外部灵敏度崩塌） |
| 外部验证 AUC | 0.81 | **0.764** |
| 外部验证灵敏度 | 0.94 | **0.919** |
| 可解释性 | 无 | **逐例 SHAP 归因 + 全局特征重要性** |

> ⚠️ 注意：XGBoost 输出的概率校准尺度与决策树不同，因此 cutoff 远小于旧版。
> **切勿**沿用旧版的 0.2995。
